...
bool Popup::mouseMove(QMouseEvent *e) {
  if (leftButtonPressed) {
    if (dragStart) {
      ...
      if (e->globalPos().x() >= geometry.right() - verticalScrollBar()->width())
        return false;
      if (e->globalPos().y() >= geometry.bottom() - horizontalScrollBar()->height())
        return false;
      int newW = e->pos().x() - dragPos.x() + startWidth;
      int newH = e->pos().y() - dragPos.y() + startHeight;
      if (newW <= verticalScrollBar()->width())
        return false;
      if (newH <= horizontalScrollBar()->height())
        return false;
      resize(newW, newH);
      return true;
    }
